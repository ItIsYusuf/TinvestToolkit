...
class Scheduler:
    ...
    def stop(self):
        ...
        self._logger.info('Scheduler has stopped!')

    def add_tasks(self, tasks: list[SyncTask] | list[AsyncTask]) -> None:
        if tasks:
            self._prepare_tasks(*tasks)
            self._logger.info(f'Tasks added to the scheduler in quantity: [{len(tasks)}]!')
        else:
            self._logger.info('Added task list is empty...')
    ...
    def delete_tasks_by_unique_name(self, names: list[str]) -> None:
        ...
        self._logger.info(f'Tasks have been deleted in quantity: [{len(names)}]')

    def _run_async_helpers(self):
        self._logger.debug('Running helper threads for async tasks...')
        ...
        self._logger.debug('Launch success!')
    ...
    def _start_task_checker(self, async_futures_q: Queue):
        while True:
            ...
            if exception is not None:
                ...
                self._logger.error(
                    f'Task ended with error. UUID - [{task._instance_key}].\n {exception_formated}')
            else:
                self._logger.info(f'Task ended. UUID - [{task._instance_key}]')
